...
public class AfterDeliveryUnApproveRule implements IRule<DeliveryVO> {

	  @Override
	  public void process(DeliveryVO[] vos) {
		  try {
		      String sourceid = BusinessEventType.getSourceIDByBillVO(vos);
		      EventDispatcher.fireEvent(new BdUpdateEvent(sourceid, IEventType.TYPE_UNAPPROV_AFTER,vos,null));
		    }
		    catch (Exception e) {
